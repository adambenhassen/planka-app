...
void main() {
  ...
  Future<void> submitLoginAndWait(WidgetTester tester) async {
    ...
    while (DateTime.now().isBefore(deadline)) {
      ...
      // A simulator can deliver keyboard dismissal after the first tap. Keep
      // retrying only while the live login tree is present, so no retry can
      // target a stale button after navigation.
  ...
  testWidgets('capture README screenshots', (tester) async {
    ...
    await tester.drag(cardSheetScroll, const Offset(0, 225));
    await tester.pumpAndSettle();
